...
class VarHolder<T, typename std::enable_if< !std::is_class<T>::value >::type> : public VarBase {
 ...
 VarHolder( const T& val ) : m_var( val ) {}
  ...
  void print() const {
    std::cout << "value = " << m_var << " [ " << print_type(m_var) << " ], address = " << &m_var  << ", size = " << sizeof( m_var ) << std::endl;
  }
...
class VarHolder<T, typename std::enable_if< is_vector<T>::value >::type> : public VarBase {
 ...
 VarHolder( const T& val ) : m_var( val ) {}
  ...
  void print() const {
    std::cout << "vector size = " << m_var.size() << ", type name = [ " << print_type(m_var) << " ], address = " << &m_var  << ", size = " << sizeof( m_var ) << std::endl;
  }
...
class VarHolder<T, typename std::enable_if< std::is_class<T>::value && !is_vector<T>::value >::type> : public VarBase {
 ...
 VarHolder( const T& val ) : m_var( val ) {}
  ...
  void print() const {
    std::cout << m_var << ", type name = [ " << print_type( m_var ) << " ], address = " << &m_var << ", size = " << sizeof( m_var ) << std::endl;
  }
...
*/
template< typename T>
class VarHolder<T, typename std::enable_if< std::is_class<T>::value && (!is_vector<T>::value) && std::is_base_of<TH1, T>::value >::type > : public VarBase {
 public:
 VarHolder( const T& val ) : m_var( val ) {}
  ~VarHolder(){}
  const std::type_info& type() const { return typeid(T); }
  ...
  void print() const {
    std::cout << m_var.GetName() << ", type name = [ " << typeid(std::vector< T >).name() << " ], address = " << &m_var  << ", size = " << sizeof( m_var ) << std::endl;
  }
