...
#define ERRMSG_UNCONFIG( NAME ) std::cerr << "ERROR"  << __PRETTY_FUNCTION__ << ": unconfigured (" << #NAME << ")" << std::endl;
#define ERRMSG_TYPE( NAME, TYPE ) std::cerr << "ERROR"  << __PRETTY_FUNCTION__ << ": type error (" << #NAME << ", " << demangle( typeid(TYPE).name() ) << ")" << std::endl;
  
  
  //____________________________________________________________________________________________________
  bool chi2Filter( const xAOD::Vertex*, const xAOD::TrackParticle* trk, const xAOD::Vertex*, const Configurator& config) {
  ...
  //____________________________________________________________________________________________________
  bool hitPatternFilter( const xAOD::Vertex* dv, const xAOD::TrackParticle* trk, const xAOD::Vertex* pv, const Configurator& config) {
    ...
    if( config.at( Config::hitPatternCondition ).type() != typeid( VsiTool::HitPatternCondition ) ) {
    ...
    const auto condition = config.at( Config::hitPatternCondition ).getVar<VsiTool::HitPatternCondition>();
  ...
  //____________________________________________________________________________________________________
  bool ipWrtSVFilter( const xAOD::Vertex*, const xAOD::TrackParticle* trk, const xAOD::Vertex*, const Configurator& config) {
    ...
    ////////////////////
    // lambda
    auto ipsignif_wrtSVCut = [&]( const xAOD::TrackParticle *trk, const enum Config& cfgname ) -> bool {
      ...
      if( cfgname == Config::d0signif_wrtSVCut ) {
        name = "d0";
      }
      else if( cfgname == Config::z0signif_wrtSVCut ) {
        name = "z0";
      }
      else {
        std::cerr << "ERROR: " << __FUNCTION__ << ": invalid config type specified!" << std::endl;
        throw( Error::TypeError );
      }
      
      // find variables
      if( !trk->isAvailable<float>( name + "_wrtSV" ) ) {
        std::cerr << "WARNING: " << __FUNCTION__ << ": auxdyn variable " << ( name + "_wrtSV" ) << " not available!" << std::endl;
        return true;
      }
      if( !trk->isAvailable<float>( "err" + name + "_wrtSV" ) ) {
        std::cerr << "WARNING: " << __FUNCTION__ << ": auxdyn variable " << ( "err" + name + "_wrtSV" ) << " not available!" << std::endl;
        return true;
      }
    ...
    if( !varWrtSVCut<Config::chi2_toSVCut>   ( trk, config, "chi2_toSV" ) )       return false;
  ...
  //____________________________________________________________________________________________________
  bool dropAssociated( const xAOD::Vertex*, const xAOD::TrackParticle* trk, const xAOD::Vertex*, const Configurator& config) {
  ...
  //____________________________________________________________________________________________________
  bool dropNonSelected( const xAOD::Vertex*, const xAOD::TrackParticle* trk, const xAOD::Vertex*, const Configurator& config) {
    ...
    auto is_assoc = trk->auxdataConst<char>("is_selected");
...
#define AUXDYN( obj, type, varname ) ( obj->isAvailable<type>(varname)? obj->auxdataConst<type>(varname) : AlgConsts::invalidFloat )
  ...
  TLorentzVector sumP4( const std::vector<const xAOD::TrackParticle*>& tracks ) {
    ...
    for( auto* trk : tracks ) {
      if( !trk ) continue;
        TLorentzVector vv;
        vv.SetPtEtaPhiM( AUXDYN( trk, float, "pt_wrtSV" ), AUXDYN( trk, float, "eta_wrtSV" ), AUXDYN( trk, float, "phi_wrtSV" ),  PhysConsts::mass_chargedPion );
  ...
  double minOpAng( const std::vector<const xAOD::TrackParticle*>& tracks ) {
    ...
    for( auto i_itr = tracks.begin(); i_itr != tracks.end(); ++i_itr ) {
      ...
      TVector3 iv; iv.SetPtEtaPhi(1.0, AUXDYN( (*i_itr), float, "eta_wrtSV" ), AUXDYN( (*i_itr), float, "phi_wrtSV" ) );
      
      for( auto j_itr = std::next( i_itr ); j_itr != tracks.end(); ++j_itr ) {
        
        TVector3 jv; jv.SetPtEtaPhi(1.0, AUXDYN( (*j_itr), float, "eta_wrtSV" ), AUXDYN( (*j_itr), float, "phi_wrtSV" ) );
  ...
  double mind0( const std::vector<const xAOD::TrackParticle*>& tracks ) {
    ...
    auto max = std::min_element( tracks.begin(), tracks.end(), []( const auto* trk1, const auto *trk2 ) { return fabs( trk1->d0() ) < fabs( trk2->d0() ); } );
  ...
  double maxd0( const std::vector<const xAOD::TrackParticle*>& tracks ) {
    ...
    auto max = std::max_element( tracks.begin(), tracks.end(), []( const auto* trk1, const auto *trk2 ) { return fabs( trk1->d0() ) < fabs( trk2->d0() ); } );
  ...
  double minOneTrackRemovedMass( const std::vector<const xAOD::TrackParticle*>& tracks ) {
    ...
    for( auto* trk : tracks ) {
      ...
      auto wrkTrks = tracks;
      wrkTrks.erase( std::remove_if( wrkTrks.begin(), wrkTrks.end(), [trk](const xAOD::TrackParticle* t) { return trk == t; } ), wrkTrks.end() );
      
      auto vv = std::accumulate( wrkTrks.begin(), wrkTrks.end(), TLorentzVector{},
                                 []( TLorentzVector& v, const xAOD::TrackParticle* trk ) {
                                   TLorentzVector tmp;
                                   tmp.SetPtEtaPhiM( AUXDYN( trk, float, "pt_wrtSV" ), AUXDYN( trk, float, "eta_wrtSV" ), AUXDYN( trk, float, "phi_wrtSV" ),  PhysConsts::mass_chargedPion );
                                   v += tmp; return v; } );
  ...
  //____________________________________________________________________________________________________
  std::vector< std::tuple<double, double, std::pair<const xAOD::TrackParticle*, const xAOD::TrackParticle*> > >
  allTwoTracksMassCombinations( const std::vector<const xAOD::TrackParticle*>& tracks ) {
    
    using Tuple = std::tuple<double, double, std::pair<const xAOD::TrackParticle*, const xAOD::TrackParticle*> >;
    std::vector< Tuple > container;
    
    for( auto i_itr = tracks.begin(); i_itr != tracks.end(); ++i_itr ) {
      
      const auto* itrk = (*i_itr);
      
      for( auto j_itr = std::next( i_itr ); j_itr != tracks.end(); ++j_itr ) {
        
        const auto* jtrk = (*j_itr);
        ...
        for( const auto* trk : tracks ) {
          ...
          trk_p4.SetPtEtaPhiM( AUXDYN( trk, float, "pt_wrtSV" ), AUXDYN( trk, float, "eta_wrtSV" ), AUXDYN( trk, float, "phi_wrtSV" ),  PhysConsts::mass_chargedPion );
          
          if( (trk == itrk) || (trk == jtrk) ) {
        ...
        auto tuple = std::make_tuple( v4_2trk.M(), v4_rest.M(), std::make_pair( itrk, jtrk ) );
  ...
  //____________________________________________________________________________________________________
  std::vector<std::vector<TLorentzVector> > kTalg( const std::vector<const xAOD::TrackParticle*>& tracks, const double& R, const double& power, const double& threshold ) {

    using namespace asg::msgUserCode;
    
    std::vector< std::vector<TLorentzVector> > clusters;
    for( auto* trk : tracks ) {
      TLorentzVector v; v.SetPtEtaPhiM( AUXDYN( trk, float, "pt_wrtSV" ), AUXDYN( trk, float, "eta_wrtSV" ), AUXDYN( trk, float, "phi_wrtSV" ),  PhysConsts::mass_chargedPion );
      if( !trk->isAvailable<float>("pt_wrtSV") ) {
    ...
    for( size_t i=0; i< clusters.size(); i++ ) {
      auto& icluster = clusters.at(i);
        
      for( size_t j=0; j< clusters.size(); j++ ) {
        auto& jcluster = clusters.at(j);

        if( i > j ) continue;

        auto isum = sumP4( icluster );
        auto jsum = sumP4( jcluster );
        ...
        if( i == j ) {
          ...
          auto dist = std::min( pow(isum.Pt(), power), pow(jsum.Pt(), power) )*dRrelSq;
          
          //std::cout << Form(" (%.2f, %.2f) -> dR = %.3f, R = %.3f, dRrelSq = %.3f, dist = %.3e", isum.Pt(), jsum.Pt(), hypot(deta, dphi), R, dRrelSq, dist ) << std::endl;
    ...
    while( blist.size()>0 ) {
      ...
      auto min_dij = std::min_element( dlist.begin(), dlist.end(),
                                       []( std::pair< std::pair<unsigned, unsigned>, double> p1, std::pair< std::pair<unsigned, unsigned>, double> p2 ) {
                                         return p1.second < p2.second;
                                       } );
      auto min_bi  = std::min_element( blist.begin(), blist.end(),
                                       []( std::pair< unsigned, double> p1, std::pair< unsigned, double> p2 ) {
                                         return p1.second < p2.second;
                                       } );
    
      //ANA_MSG_INFO(  __FUNCTION__ << ": min_dij = " << (*min_dij).second << ", min_bi  = " << (*min_bi).second );
      ...
      if( (*min_dij).second < (*min_bi).second ) {
        ...
        for( auto& pair : dlist ) {
          if( pair.first.first == i ) {
            auto& icluster = clusters.at(i);
            auto& kcluster = clusters.at( pair.first.second );
            auto isum = sumP4( icluster );
            auto ksum = sumP4( kcluster );
          ...
          if( pair.first.second == i ) {
            auto& icluster = clusters.at(i);
            auto& kcluster = clusters.at( pair.first.first );
            auto isum = sumP4( icluster );
            auto ksum = sumP4( kcluster );
            auto dR = hypot( isum.Eta()-ksum.Eta(), VsiTool::dPhiMod( isum.Phi(), ksum.Phi() ) );
            pair.second = std::min( pow(isum.Pt(), power), pow(ksum.Pt(), power) )*dR/R;
        ...
        for( auto itr = dlist.begin(); itr != dlist.end(); ) {
          if( (*itr).first.first == j || (*itr).first.second == j ) {
            dlist.erase( itr++ );
          } else {
            ++itr;
          }
        ...
        for( auto itr = dlist.begin(); itr != dlist.end(); ) {
          if( (*itr).first.first == i || (*itr).first.second == i ) {
            dlist.erase( itr++ );
          } else {
            ++itr;
          }
      ...
      //ANA_MSG_INFO( __FUNCTION__ << ": --> jet size = " << jets.size() << ", input tracks size = " << tracks.size() << ", dlist size = " << dlist.size() << ", blist size = " << blist.size() );
    }
    
    jets.erase( std::remove_if( jets.begin(), jets.end(),
                                [&]( std::vector<TLorentzVector>& j ) { auto sumP = sumP4( j ); return ( sumP.Pt() < threshold ); } ),
                jets.end() );
...
namespace VsiTool {
  ...
  const TVector3 dvDirection( const xAOD::Vertex* dv, const xAOD::Vertex* pv ) {
    ...
    auto& pos_dv = dv->position();
    auto& pos_pv = pv->position();
  
    auto diff = pos_dv - pos_pv;
  ...
  template<> bool patternMatchInOut<VertexRegion::insideBeamPipe>( const uint32_t& pattern ) {
    if( ! (pattern & (1<<VsiTool::pixelBarrel0)) ) return false;
    if( ! (pattern & (1<<VsiTool::pixelBarrel1)) ) return false;
    return true;
  }
  
  template<> bool patternMatchInOut<VertexRegion::insidePixelBarrel0>( const uint32_t& pattern ) {
    if( ! (pattern & (1<<VsiTool::pixelBarrel0)) ) return false;
    if( ! (pattern & (1<<VsiTool::pixelBarrel1)) ) return false;
    return true;
  }
  
  template<> bool patternMatchInOut<VertexRegion::aroundPixelBarrel0>( const uint32_t& pattern ) {
    if( ! (pattern & (1<<VsiTool::pixelBarrel1)) ) return false;
    if( ! (pattern & (1<<VsiTool::pixelBarrel2)) ) return false;
    return true;
  }
  
  template<> bool patternMatchInOut<VertexRegion::outsidePixelBarrel0_and_insidePixelBarrel1>( const uint32_t& pattern ) {
    if(   (pattern & (1<<VsiTool::pixelBarrel0)) ) return false;
    if( ! (pattern & (1<<VsiTool::pixelBarrel1)) ) return false;
    if( ! (pattern & (1<<VsiTool::pixelBarrel2)) ) return false;
    return true;
  ...
  template<> bool patternMatchInOut<VertexRegion::aroundPixelBarrel1>( const uint32_t& pattern ) {
    if(   (pattern & (1<<VsiTool::pixelBarrel0)) ) return false;
    if( ! (pattern & (1<<VsiTool::pixelBarrel2)) ) return false;
  ...
  template<> bool patternMatchInOut<VertexRegion::outsidePixelBarrel1_and_insidePixelBarrel2>( const uint32_t& pattern ) {
    if(   (pattern & (1<<VsiTool::pixelBarrel0)) ) return false;
    if(   (pattern & (1<<VsiTool::pixelBarrel1)) ) return false;
    if( ! (pattern & (1<<VsiTool::pixelBarrel2)) ) return false;
    if( ! (pattern & (1<<VsiTool::pixelBarrel3)) ) return false;
  ...
  template<> bool patternMatchInOut<VertexRegion::aroundPixelBarrel2>( const uint32_t& pattern ) {
    if(   (pattern & (1<<VsiTool::pixelBarrel0)) ) return false;
    if(   (pattern & (1<<VsiTool::pixelBarrel1)) ) return false;
    if( ! (pattern & (1<<VsiTool::pixelBarrel3)) ) return false;
    return true;
  ...
  template<> bool patternMatchInOut<VertexRegion::outsidePixelBarrel2_and_insidePixelBarrel3>( const uint32_t& pattern ) {
    if(   (pattern & (1<<VsiTool::pixelBarrel0)) ) return false;
    if(   (pattern & (1<<VsiTool::pixelBarrel1)) ) return false;
    if(   (pattern & (1<<VsiTool::pixelBarrel2)) ) return false;
    if( ! (pattern & (1<<VsiTool::pixelBarrel3)) ) return false;
  ...
  template<> bool patternMatchInOut<VertexRegion::aroundPixelBarrel3>( const uint32_t& pattern ) {
    if(   (pattern & (1<<VsiTool::pixelBarrel0)) ) return false;
    if(   (pattern & (1<<VsiTool::pixelBarrel1)) ) return false;
    if(   (pattern & (1<<VsiTool::pixelBarrel2)) ) return false;
  ...
  template<> bool patternMatchInOut<VertexRegion::outsidePixelBarrel3_and_insideSctBarrel0>( const uint32_t& pattern ) {
    if(   (pattern & (1<<VsiTool::pixelBarrel0)) ) return false;
    if(   (pattern & (1<<VsiTool::pixelBarrel1)) ) return false;
    if(   (pattern & (1<<VsiTool::pixelBarrel2)) ) return false;
    if(   (pattern & (1<<VsiTool::pixelBarrel3)) ) return false;
  ...
  template<> bool patternMatchInOut<VertexRegion::aroundSctBarrel0>( const uint32_t& pattern ) {
    if(   (pattern & (1<<VsiTool::pixelBarrel0)) ) return false;
    if(   (pattern & (1<<VsiTool::pixelBarrel1)) ) return false;
    if(   (pattern & (1<<VsiTool::pixelBarrel2)) ) return false;
    if(   (pattern & (1<<VsiTool::pixelBarrel3)) ) return false;
  ...
  template<> bool patternMatchInOut<VertexRegion::outsideSctBarrel0_and_insideSctBarrel1>( const uint32_t& pattern ) {
    if(   (pattern & (1<<VsiTool::pixelBarrel0)) ) return false;
    if(   (pattern & (1<<VsiTool::pixelBarrel1)) ) return false;
    if(   (pattern & (1<<VsiTool::pixelBarrel2)) ) return false;
    if(   (pattern & (1<<VsiTool::pixelBarrel3)) ) return false;
  ...
  template<> bool patternMatchInOut<VertexRegion::aroundSctBarrel1>( const uint32_t& pattern ) {
    if(   (pattern & (1<<VsiTool::pixelBarrel0)) ) return false;
    if(   (pattern & (1<<VsiTool::pixelBarrel1)) ) return false;
    if(   (pattern & (1<<VsiTool::pixelBarrel2)) ) return false;
    if(   (pattern & (1<<VsiTool::pixelBarrel3)) ) return false;
  ...
  template<> bool patternMatchInOut<VertexRegion::elsewhere>( const uint32_t& /*pattern*/ ) {
    return true;
  }
  ...
  template<> bool patternMatchOuterOnly<VertexRegion::insideBeamPipe>( const uint32_t& pattern ) {
    if( ! (pattern & (1<<VsiTool::pixelBarrel0)) ) return false;
    if( ! (pattern & (1<<VsiTool::pixelBarrel1)) ) return false;
    return true;
  }
  
  template<> bool patternMatchOuterOnly<VertexRegion::insidePixelBarrel0>( const uint32_t& pattern ) {
    if( ! (pattern & (1<<VsiTool::pixelBarrel0)) ) return false;
    if( ! (pattern & (1<<VsiTool::pixelBarrel1)) ) return false;
    return true;
  }
  
  template<> bool patternMatchOuterOnly<VertexRegion::aroundPixelBarrel0>( const uint32_t& pattern ) {
    if( ! (pattern & (1<<VsiTool::pixelBarrel1)) ) return false;
    if( ! (pattern & (1<<VsiTool::pixelBarrel2)) ) return false;
    return true;
  }
  
  template<> bool patternMatchOuterOnly<VertexRegion::outsidePixelBarrel0_and_insidePixelBarrel1>( const uint32_t& pattern ) {
    if( ! (pattern & (1<<VsiTool::pixelBarrel1)) ) return false;
    if( ! (pattern & (1<<VsiTool::pixelBarrel2)) ) return false;
    return true;
  }
  
  template<> bool patternMatchOuterOnly<VertexRegion::aroundPixelBarrel1>( const uint32_t& pattern ) {
    if( ! (pattern & (1<<VsiTool::pixelBarrel2)) ) return false;
    if( ! (pattern & (1<<VsiTool::pixelBarrel3)) ) return false;
    return true;
  }
  
  template<> bool patternMatchOuterOnly<VertexRegion::outsidePixelBarrel1_and_insidePixelBarrel2>( const uint32_t& pattern ) {
    if( ! (pattern & (1<<VsiTool::pixelBarrel2)) ) return false;
    if( ! (pattern & (1<<VsiTool::pixelBarrel3)) ) return false;
    return true;
  }
  
  template<> bool patternMatchOuterOnly<VertexRegion::aroundPixelBarrel2>( const uint32_t& pattern ) {
    if( ! (pattern & (1<<VsiTool::pixelBarrel3)) ) return false;
    return true;
  }
  
  template<> bool patternMatchOuterOnly<VertexRegion::outsidePixelBarrel2_and_insidePixelBarrel3>( const uint32_t& pattern ) {
    if( ! (pattern & (1<<VsiTool::pixelBarrel3)) ) return false;
    return true;
  }
  
  template<> bool patternMatchOuterOnly<VertexRegion::aroundPixelBarrel3>( const uint32_t& pattern ) {
    if( ! (pattern & (1<<VsiTool::sctBarrel0)) ) return false;
    if( ! (pattern & (1<<VsiTool::sctBarrel1)) ) return false;
    return true;
  }
  
  template<> bool patternMatchOuterOnly<VertexRegion::outsidePixelBarrel3_and_insideSctBarrel0>( const uint32_t& pattern ) {
    if( ! (pattern & (1<<VsiTool::sctBarrel0)) ) return false;
    if( ! (pattern & (1<<VsiTool::sctBarrel1)) ) return false;
    return true;
  }
  
  template<> bool patternMatchOuterOnly<VertexRegion::aroundSctBarrel0>( const uint32_t& pattern ) {
    if( ! (pattern & (1<<VsiTool::sctBarrel1)) ) return false;
    if( ! (pattern & (1<<VsiTool::sctBarrel2)) ) return false;
    return true;
  }
  
  template<> bool patternMatchOuterOnly<VertexRegion::outsideSctBarrel0_and_insideSctBarrel1>( const uint32_t& pattern ) {
    if( ! (pattern & (1<<VsiTool::sctBarrel1)) ) return false;
    if( ! (pattern & (1<<VsiTool::sctBarrel2)) ) return false;
    return true;
  }
  
  template<> bool patternMatchOuterOnly<VertexRegion::aroundSctBarrel1>( const uint32_t& pattern ) {
    if( ! (pattern & (1<<VsiTool::sctBarrel2)) ) return false;
    if( ! (pattern & (1<<VsiTool::sctBarrel3)) ) return false;
    return true;
  }
  
  template<> bool patternMatchOuterOnly<VertexRegion::elsewhere>( const uint32_t& /*pattern*/ ) {
    return true;
  }
  ...
  template<> bool patternMatchOuterOnlyTight<VertexRegion::insideBeamPipe>( const uint32_t& pattern ) {
    if( ! (pattern & (1<<VsiTool::pixelBarrel0)) ) return false;
    if( ! (pattern & (1<<VsiTool::pixelBarrel1)) ) return false;
    return true;
  }
  
  template<> bool patternMatchOuterOnlyTight<VertexRegion::insidePixelBarrel0>( const uint32_t& pattern ) {
    if( ! (pattern & (1<<VsiTool::pixelBarrel0)) ) return false;
    if( ! (pattern & (1<<VsiTool::pixelBarrel1)) ) return false;
    return true;
  }
  
  template<> bool patternMatchOuterOnlyTight<VertexRegion::aroundPixelBarrel0>( const uint32_t& pattern ) {
    if( ! (pattern & (1<<VsiTool::pixelBarrel1)) ) return false;
    if( ! (pattern & (1<<VsiTool::pixelBarrel2)) ) return false;
    return true;
  }
  
  template<> bool patternMatchOuterOnlyTight<VertexRegion::outsidePixelBarrel0_and_insidePixelBarrel1>( const uint32_t& pattern ) {
    if( ! (pattern & (1<<VsiTool::pixelBarrel1)) ) return false;
    if( ! (pattern & (1<<VsiTool::pixelBarrel2)) ) return false;
    return true;
  }
  
  template<> bool patternMatchOuterOnlyTight<VertexRegion::aroundPixelBarrel1>( const uint32_t& pattern ) {
    if( ! (pattern & (1<<VsiTool::pixelBarrel2)) ) return false;
    if( ! (pattern & (1<<VsiTool::pixelBarrel3)) ) return false;
    return true;
  }
  
  template<> bool patternMatchOuterOnlyTight<VertexRegion::outsidePixelBarrel1_and_insidePixelBarrel2>( const uint32_t& pattern ) {
    if( ! (pattern & (1<<VsiTool::pixelBarrel2)) ) return false;
    if( ! (pattern & (1<<VsiTool::pixelBarrel3)) ) return false;
    return true;
  }
  
  template<> bool patternMatchOuterOnlyTight<VertexRegion::aroundPixelBarrel2>( const uint32_t& pattern ) {
    if( ! hasHit( pattern, VsiTool::pixelBarrel3 ) ) return false;
    
    auto outer_sctCond1 = ( hasHit( pattern, VsiTool::sctBarrel0 ) && hasHit( pattern, VsiTool::sctBarrel1 ) );
    auto outer_sctCond2 = ( hasHit( pattern, VsiTool::sctBarrel0 ) && hasHit( pattern, VsiTool::sctEndCap0 ) );
    auto outer_sctCond3 = ( hasHit( pattern, VsiTool::sctBarrel0 ) && hasHit( pattern, VsiTool::sctEndCap1 ) && hasHit( pattern, VsiTool::sctEndCap2 ) );
    
    if( ! ( outer_sctCond1 || outer_sctCond2 || outer_sctCond3 ) ) return false;
    return true;
  }
  
  template<> bool patternMatchOuterOnlyTight<VertexRegion::outsidePixelBarrel2_and_insidePixelBarrel3>( const uint32_t& pattern ) {
    if( ! (pattern & (1<<VsiTool::pixelBarrel3)) ) return false;
  ...
  template<> bool patternMatchOuterOnlyTight<VertexRegion::aroundPixelBarrel3>( const uint32_t& pattern ) {
    
    auto outer_sctCond1 = ( hasHit( pattern, VsiTool::sctBarrel0 ) && hasHit( pattern, VsiTool::sctBarrel1 ) && hasHit( pattern, VsiTool::sctBarrel2 ) );
    auto outer_sctCond2 = ( hasHit( pattern, VsiTool::sctBarrel0 ) && hasHit( pattern, VsiTool::sctBarrel1 ) && hasHit( pattern, VsiTool::sctEndCap0 ) );
    auto outer_sctCond3 = ( hasHit( pattern, VsiTool::sctBarrel0 ) && hasHit( pattern, VsiTool::sctEndCap0 ) && hasHit( pattern, VsiTool::sctEndCap1 ) );
    auto outer_sctCond4 = ( hasHit( pattern, VsiTool::sctEndCap0 ) && hasHit( pattern, VsiTool::sctEndCap1 ) && hasHit( pattern, VsiTool::sctEndCap2 ) );
    
    if( ! ( outer_sctCond1 || outer_sctCond2 || outer_sctCond3 || outer_sctCond4 ) ) return false;
    return true;
  }
  
  template<> bool patternMatchOuterOnlyTight<VertexRegion::outsidePixelBarrel3_and_insideSctBarrel0>( const uint32_t& pattern ) {
    auto outer_sctCond1 = ( hasHit( pattern, VsiTool::sctBarrel0 ) && hasHit( pattern, VsiTool::sctBarrel1 ) && hasHit( pattern, VsiTool::sctBarrel2 ) );
    auto outer_sctCond2 = ( hasHit( pattern, VsiTool::sctBarrel0 ) && hasHit( pattern, VsiTool::sctBarrel1 ) && hasHit( pattern, VsiTool::sctEndCap0 ) );
    auto outer_sctCond3 = ( hasHit( pattern, VsiTool::sctBarrel0 ) && hasHit( pattern, VsiTool::sctEndCap0 ) && hasHit( pattern, VsiTool::sctEndCap1 ) );
    auto outer_sctCond4 = ( hasHit( pattern, VsiTool::sctEndCap0 ) && hasHit( pattern, VsiTool::sctEndCap1 ) && hasHit( pattern, VsiTool::sctEndCap2 ) );
  ...
  template<> bool patternMatchOuterOnlyTight<VertexRegion::aroundSctBarrel0>( const uint32_t& pattern ) {
    if( ! (pattern & (1<<VsiTool::sctBarrel1)) ) return false;
    if( ! (pattern & (1<<VsiTool::sctBarrel2)) ) return false;
    if( ! (pattern & (1<<VsiTool::sctBarrel3)) ) return false;
    return true;
  ...
  template<> bool patternMatchOuterOnlyTight<VertexRegion::outsideSctBarrel0_and_insideSctBarrel1>( const uint32_t& pattern ) {
    if( ! (pattern & (1<<VsiTool::sctBarrel1)) ) return false;
    if( ! (pattern & (1<<VsiTool::sctBarrel2)) ) return false;
    if( ! (pattern & (1<<VsiTool::sctBarrel3)) ) return false;
    return true;
  ...
  template<> bool patternMatchOuterOnlyTight<VertexRegion::aroundSctBarrel1>( const uint32_t& pattern ) {
    if( ! (pattern & (1<<VsiTool::sctBarrel2)) ) return false;
    if( ! (pattern & (1<<VsiTool::sctBarrel3)) ) return false;
    return true;
  }
  
  template<> bool patternMatchOuterOnlyTight<VertexRegion::elsewhere>( const uint32_t& /*pattern*/ ) {
    return true;
  }
...
#define CASEPATTERN_OUTERONLY_TIGHT( REGION ) case REGION:\
  ...
  bool passLoosePattern ( const uint32_t& pattern, const enum VertexRegion& region, const TVector3& trackRelDir ) {
  ...
  bool passMediumPattern ( const uint32_t& pattern, const enum VertexRegion& region, const TVector3& trackRelDir ) {
  ...
  bool passTightPattern ( const uint32_t& pattern, const enum VertexRegion& region, const TVector3& /* trackRelDir */ ) {
